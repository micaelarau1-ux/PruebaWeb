...
## Pendientes marcados en el HTML (opcionales)

Están comentados dentro de `index.html`, listos para activar cuando tengas los datos:

- **GA4 / Google Tag Manager** (arriba de todo) → pegá tu ID y descomentá.
- **Embed de Spotify** (sección Música) → reemplazá el bloque por el iframe real.
- **Imagen de evento** (sección Eventos) → descomentá el `<img>` del primer evento si querés mostrarla.
- **og:image / dominio** → cuando tengas la URL final de Vercel, podés poner la URL absoluta en los meta `og:image` para que el preview funcione en todas las apps.
